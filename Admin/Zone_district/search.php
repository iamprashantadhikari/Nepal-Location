...
$sqlZoneDist = "SELECT * FROM zone_district WHERE zone_id IN ('".implode("','",$zoneIds)."') OR district_id IN ('".implode("','",$districtIds)."')";
$zoneDistResult = mysqli_query($conn,$sqlZoneDist);
...
<table id="example" class="table table-striped table-bordered" style="width:100%">
    ...
    <tbody>
            <?php if (mysqli_num_rows($zoneDistResult)>0){ ?>
            <?php while ($row = mysqli_fetch_array($zoneDistResult)){ ?>
        <tr>
    ...
    <div class="data-empty">
        <?php if(mysqli_num_rows($zoneDistResult)==0) echo "No results" ?>
    </div>
</body>
</html>