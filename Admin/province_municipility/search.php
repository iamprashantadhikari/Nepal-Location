...
<body>
    ...
    $provSql = "SELECT * FROM province WHERE province LIKE '%".$search."%'";
    $provResult = mysqli_query($conn,$provSql);
    if(mysqli_num_rows($provResult)>0){
        while($provRow=mysqli_fetch_array($provResult)){
            array_push($provinceIds, $provRow['id']);
        }
    }
    ...
    $provMuniSql = "SELECT * FROM province_municipility WHERE province_id IN ('".implode("','",$provinceIds)."') OR district_id IN ('".implode("','",$districtIds)."')  OR municipility_id IN ('".implode("','",$muniIds)."')";
    $provMuniResult = mysqli_query($conn,$provMuniSql);
...
<table id="example" class="table table-striped table-bordered" style="width:100%">
    ...
    <tbody>
        <?php if(mysqli_num_rows($provMuniResult)>0){ ?>
            <?php while($row=mysqli_fetch_array($provMuniResult)){ ?>
            <tr>

                <?php
                $sql1 = "SELECT * FROM province WHERE id=$row[province_id]";
...
<div class="data-empty"> 
    <?php if(mysqli_num_rows($provMuniResult)==0) echo "No results";?> 
</div>
